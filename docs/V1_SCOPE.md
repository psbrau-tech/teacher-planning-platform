...
- Standards reconciliation follows Alabama's subject-specific, multi-year Course of Study revision pattern rather than assuming monthly change. TPP performs a comprehensive governed-catalog and source validation each July before the school year; lightweight authoritative-catalog monitoring on the first workday of January, April, and October; and event-driven reconciliation whenever an ALSDE/State Board adoption, amendment, replacement, or other authoritative change signal is identified. July replaces the third-quarter lightweight check rather than duplicating it. Detected or newly discovered changes are staged for review and never silently replace an approved source/snapshot.
- Historical plans retain the exact standards source/version that was effective and selected when the plan was created; later adoptions never rewrite validated historical evidence.
...
9. Three-layer standards reconciliation: (a) annual comprehensive catalog/source validation in July before the school year, (b) lightweight catalog monitoring on the first workday of January, April, and October, and (c) event-driven reconciliation when ALSDE or the State Board adopts, amends, replaces, or otherwise changes an authoritative Course of Study/source. Every run retains auditable unchanged/changed/new/unavailable evidence; detected changes remain pending until reviewed and approved, and source failures do not invalidate the last approved snapshot.
