...
from app.pdf_fields import ALL_HQI_FIELDS
...
def test_approved_anniston_template_field_contract() -> None:
    ...
    assert set(actual_fields) == set(ALL_HQI_FIELDS), (
        "The approved PDF field names do not match the application contract. "
        f"Expected={sorted(ALL_HQI_FIELDS)} Actual={sorted(actual_fields)}"
    )
