        ...
        sources = _records(
            _request(
                ...
                params={
                    ...
                    "select": "id,source_key,title,source_kind",
                },
            )
        )
    ...
    for snapshot in snapshots:
        ...
        results.append(
            PendingSnapshotRead(
                ...
                parser_version=_optional_text(snapshot, "parser_version"),
                retrieved_at=_text(snapshot, "retrieved_at"),
                resolved_document_url=_text(snapshot, "resolved_document_url"),
...
@router.post(
    ...
    response_model=SnapshotApprovalRead,
)
        ...
        _request(
            ...
            params={
                "select": (
                    ...
                    "discovered_source_count,unchanged_count,changed_count,new_count,"
                    "missing_count,error_summary"
                ),
...
def get_catalog_run(
    ...
    return CatalogRunDetailRead(
        run=_catalog_run(run_rows[0]),
        items=[
            CatalogItemRead(
                id=_uuid(row, "id"),
                source_key=_text(row, "source_key"),
                result_state=_text(row, "result_state"),
                family=_text(row, "family"),
                category_name=_optional_text(row, "category_name"),
                authority=_text(row, "authority"),
                observed_title=_optional_text(row, "observed_title"),
                observed_edition=_optional_text(row, "observed_edition"),
                observed_document_url=_optional_text(row, "observed_document_url"),
                previous_title=_optional_text(row, "previous_title"),
                previous_edition=_optional_text(row, "previous_edition"),
                previous_document_url=_optional_text(row, "previous_document_url"),
            )
...
def _catalog_run(row: JsonRecord) -> CatalogRunRead:
    return CatalogRunRead(
        ...
        check_month=_optional_text(row, "check_month"),
        trigger_kind=_text(row, "trigger_kind"),
        status=_text(row, "status"),
        catalog_sha256=_text(row, "catalog_sha256"),
        discovered_source_count=_int(row, "discovered_source_count"),
        unchanged_count=_int(row, "unchanged_count"),
        changed_count=_int(row, "changed_count"),
        new_count=_int(row, "new_count"),
        missing_count=_int(row, "missing_count"),
