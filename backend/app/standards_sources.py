...
def resolve_authoritative_document(
    ...
    *,
    ...
    if not content or len(content) > MAX_LANDING_BYTES:
        raise StandardsSourceResolutionError(
...
def _resolve_alabama_bma(
    ...
    matches = [
        ...
        for anchor in anchors
        if anchor.href.lower().endswith(".pdf")
        and "course-of-study" in anchor.href.lower()
        and (
            "bma" in anchor.href.lower()
            or "business management" in anchor.text.lower()
            or "business management" in anchor.href.lower()
        )
    ]
    if not matches:
