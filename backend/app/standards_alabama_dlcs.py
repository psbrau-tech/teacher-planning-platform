...
_DEVELOPMENTAL = re.compile(
    r"developmentally appropriate beginning in Grade\s+(\d+)", flags=re.IGNORECASE
)
_THEMES = {
...
_BANDS = (
    _Band("Kindergarten Grade 1 Grade 2", ("kindergarten", "grade_1", "grade_2"), ("Kindergarten", "Grade 1", "Grade 2"), ("K", "1", "2"), (15, 17, 20)),
    _Band("Grade 3 Grade 4 Grade 5", ("grade_3", "grade_4", "grade_5"), ("Grade 3", "Grade 4", "Grade 5"), ("3", "4", "5"), (19, 26, 24)),
    _Band("Grade 6 Grade 7 Grade 8", ("grade_6", "grade_7", "grade_8"), ("Grade 6", "Grade 7", "Grade 8"), ("6", "7", "8"), (31, 32, 36)),
    _Band("Grades 9-12", ("grades_9_12",), ("Grades 9-12",), ("9-12",), (45,)),
)
...
def parse_alabama_dlcs_2025(extracted: ExtractedDocument) -> ParsedStandardsDocument:
    ...
    next_number: dict[str, int] = {course_key: 1 for course_key in standards_by_course}
        ...
        if current_course_key is not None and current_code is not None:
            ...
            if text:
                standards_by_course[current_course_key].append(
                    ParsedStandard(code=current_code, text=text, parent_code=None, strand=current_theme)
                )
    ...
    for raw_line in extracted.lines:
        ...
        if developmental is not None and active_band is not None:
            minimum_lane = _minimum_lane_for_grade(active_band, developmental.group(1))
        ...
        if tokens and active_band is not None:
            ...
            for token_index, token in enumerate(tokens):
                ...
                lane = _resolve_lane(active_band, minimum_lane=minimum_lane, number=number, next_number=next_number)
                if lane is None:
                    raise StandardsIngestError(
                        f"Alabama DLCS could not assign standard {number} within {active_band.header}"
                    )
    ...
    for band in _BANDS:
        for lane, course_key in enumerate(band.course_keys):
            ...
            if codes != list(range(1, expected_final + 1)):
                raise StandardsIngestError(
                    f"Alabama DLCS {band.display_names[lane]} expected standards 1 through {expected_final}"
                )
            courses.append(
                ParsedCourse(
...
def _resolve_lane(
    band: _Band, *, minimum_lane: int, number: int, next_number: dict[str, int]
) -> int | None:
...
def _minimum_lane_for_grade(band: _Band, grade: str) -> int:
    for lane, grade_band in enumerate(band.grade_bands):
        if grade_band == grade:
            return lane
    raise StandardsIngestError(
        f"Alabama DLCS developmental-grade notice {grade} does not match {band.header}"
    )
