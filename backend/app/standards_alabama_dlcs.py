...
DLCS_PARSER_VERSION = "gate-e-alabama-dlcs-2025-v1"
...
_BANDS = (
    _Band(
        header="Kindergarten Grade 1 Grade 2",
        course_keys=("kindergarten", "grade_1", "grade_2"),
        display_names=("Kindergarten", "Grade 1", "Grade 2"),
        grade_bands=("K", "1", "2"),
        expected_final=(15, 17, 20),
    ),
    _Band(
        header="Grade 3 Grade 4 Grade 5",
        course_keys=("grade_3", "grade_4", "grade_5"),
        display_names=("Grade 3", "Grade 4", "Grade 5"),
        grade_bands=("3", "4", "5"),
        expected_final=(19, 26, 24),
    ),
    _Band(
        header="Grade 6 Grade 7 Grade 8",
        course_keys=("grade_6", "grade_7", "grade_8"),
        display_names=("Grade 6", "Grade 7", "Grade 8"),
        grade_bands=("6", "7", "8"),
        expected_final=(31, 32, 36),
    ),
    _Band(
        header="Grades 9-12",
        course_keys=("grades_9_12",),
        display_names=("Grades 9-12",),
        grade_bands=("9-12",),
        expected_final=(45,),
    ),
)
...
def parse_alabama_dlcs_2025(extracted: ExtractedDocument) -> ParsedStandardsDocument:
    ...
    for raw_line in extracted.lines:
        ...
        band = _BAND_BY_HEADER.get(normalized)
        if band is not None:
            flush()
            active_band = band
            lane_cursor = 0
            ...
            for token_index, token in enumerate(tokens):
                ...
                if lane is None:
                    raise StandardsIngestError(
                        f"Alabama DLCS could not assign standard {number} within "
                        f"{active_band.header}"
                    )
                lane_cursor = lane
        ...
        if skipping_example:
            continue
        if current_code is None or _noise(line):
            continue
    ...
    for band in _BANDS:
        for lane, course_key in enumerate(band.course_keys):
            ...
            if codes != list(range(1, expected_final + 1)):
                raise StandardsIngestError(
                    f"Alabama DLCS {band.display_names[lane]} expected standards "
                    f"1 through {expected_final}"
                )
            courses.append(
                ParsedCourse(
...
def _resolve_lane(
    ...
    *,
    ...
    first_lane = max(lane_cursor, minimum_lane)
    for lane in range(first_lane, len(band.course_keys)):
