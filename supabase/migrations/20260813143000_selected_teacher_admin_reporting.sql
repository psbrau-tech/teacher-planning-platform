-- Scope Administration usage reporting to an explicit teacher selection when supplied.
-- This keeps the administrator summary aligned with the teacher filter used in the UI.

create or replace function public.admin_usage_for_period_selected(
  target_start date,
  target_end date,
...
as $$
  with selected_teachers as (
    select distinct pr.profile_id as teacher_id
    from public.profile_roles pr
    join public.profiles p on p.id = pr.profile_id
    where pr.school_id = target_school_id
      and pr.role = 'teacher'
      and p.is_active
      and pr.profile_id = any(target_teacher_ids)
  ),
  teacher_summary as (
    select count(*)::bigint as teachers_configured
  ...
  assignment_summary as (
    ...
    join selected_teachers st on st.teacher_id = ta.teacher_id
...
revoke all on function public.admin_usage_for_period_selected(date, date, uuid, uuid[]) from public, anon;
grant execute on function public.admin_usage_for_period_selected(date, date, uuid, uuid[]) to authenticated;

comment on function public.admin_usage_for_period_selected(date, date, uuid, uuid[]) is
  'Period-scoped educator planning activity summary restricted to the explicitly selected active teachers in the governed school.';
