...
returns table (
  ...
  onboarding_reflection_teacher_weeks integer,
  median_onboarding_reflection_seconds integer,
  steady_state_reflection_teacher_weeks integer,
  median_steady_state_reflection_seconds integer
)
language sql
stable
security definer
...
with bounds as (
  ...
  select
    coalesce((
      ...
      from course_by_teacher
    ), 0)::integer as median_course_setup_seconds_per_teacher,
    coalesce((
      select percentile_disc(0.5) within group (order by seconds)
    ...
    coalesce((
      select percentile_disc(0.5) within group (order by seconds)
      from workflow_by_teacher_week
      where event_key = 'active_reflection_30s'
    ...
    (select count(*)::integer from workflow_by_teacher_week
      where event_key = 'active_reflection_30s' and onboarding)
      as onboarding_reflection_teacher_weeks,
    coalesce((
      select percentile_disc(0.5) within group (order by seconds)
      from workflow_by_teacher_week
      where event_key = 'active_reflection_30s' and onboarding
    ), 0)::integer as median_onboarding_reflection_seconds,
    (select count(*)::integer from workflow_by_teacher_week
      where event_key = 'active_reflection_30s' and not onboarding)
      as steady_state_reflection_teacher_weeks,
    coalesce((
      select percentile_disc(0.5) within group (order by seconds)
      from workflow_by_teacher_week
      where event_key = 'active_reflection_30s' and not onboarding
    ), 0)::integer as median_steady_state_reflection_seconds
)
select
  b.start_date,
  b.end_date,
  ...
  coalesce(m.onboarding_reflection_teacher_weeks, 0),
  coalesce(m.median_onboarding_reflection_seconds, 0),
  coalesce(m.steady_state_reflection_teacher_weeks, 0),
  coalesce(m.median_steady_state_reflection_seconds, 0)
from bounds b
cross join totals t
