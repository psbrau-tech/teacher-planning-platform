...
export function StandardsAdministrationPanel({ accessToken, disabled = false }: StandardsAdministrationPanelProps) {
  ...
  const inspectRun = async (runId: string) => { setError(null); try { const response = await fetch(`/api/v1/standards-admin/catalog-runs/${encodeURIComponent(runId)}`, { headers }); if (!response.ok) throw new Error(await readError(response, "Catalog discovery run could not be loaded.")); setSelectedRun(await response.json() as CatalogRunDetail); } catch (caught) { setError(caught instanceof Error ? caught.message : "Catalog discovery run could not be loaded."); } };
  ...
  return (
    <section className="standards-admin-panel" aria-labelledby="standards-admin-heading">
      ...
      <section><div className="section-heading compact"><div><h3>Governed sources</h3><p className="supporting">Compact source list; expand an item when provenance details are needed.</p></div></div><div className="governance-list">{sources.map((source) => <details className="governance-source" key={source.id}><summary><span><strong>{source.catalog_category_name ?? source.title}</strong><small>{source.source_kind.replaceAll("_", " ")} · {source.discovery_status}</small></span></summary><div className="guidance-card"><p><strong>Source:</strong> {source.title}</p><p><strong>Authority:</strong> {source.authority}</p><p><strong>Edition:</strong> {source.edition}</p><p><strong>Catalog role:</strong> {source.provides_standard_entries ? "Provides teacher-visible standards entries" : "Course/reference listing only"}</p><p><strong>Approved snapshot:</strong> {source.approved_snapshot_retrieved_at ? new Date(source.approved_snapshot_retrieved_at).toLocaleDateString() : "None approved"}</p></div></details>)}</div></section>
      <section><div className="section-heading compact"><div><h3>Pending snapshots</h3><p className="supporting">This is the human-approval queue for a newly detected authoritative source version. A pending snapshot cannot affect teachers until the Platform Owner approves that exact parsed snapshot.</p></div></div>{pending.length === 0 ? <div className="empty-state"><p>No authoritative source changes are waiting for approval. Current approved snapshots remain in effect.</p></div> : <div className="governance-list">{pending.map((snapshot) => <details className="governance-source" key={snapshot.id} open><summary><span><strong>{snapshot.source_title}</strong><small>{snapshot.parser_status ?? "unknown parser status"} · {snapshot.course_count} courses · {snapshot.standard_entry_count} standards</small></span></summary><div className="guidance-card"><p><strong>Retrieved:</strong> {new Date(snapshot.retrieved_at).toLocaleDateString()}</p><p>Source version: {snapshot.source_version ?? "not published"}</p><p>Parser: {snapshot.parser_version ?? "not available"}</p>{snapshot.parser_error ? <p className="error-message">{snapshot.parser_error}</p> : null}<a href={snapshot.resolved_document_url} target="_blank" rel="noreferrer">Inspect authoritative document</a><button type="button" className="primary" disabled={disabled || workingSnapshot !== null || snapshot.parser_status !== "parsed" || snapshot.course_count === 0} onClick={() => void approve(snapshot)}>{workingSnapshot === snapshot.id ? "Approving…" : "Approve exact snapshot"}</button></div></details>)}</div>}</section>
      <section><div className="section-heading compact"><div><h3>Catalog discovery history</h3><p className="supporting">Monthly and manual checks across the complete Alabama catalog.</p></div></div><div className="grid">{runs.map((run) => <article className="card" key={run.id}><div className="card-row"><span className="badge">{run.trigger_kind}</span><span className="status">{run.status}</span></div><h3>{new Date(run.checked_at).toLocaleDateString()}</h3><p>{run.discovered_source_count} discovered · {run.unchanged_count} unchanged</p><p>{run.changed_count} changed · {run.new_count} new · {run.missing_count} missing</p>{run.error_summary ? <p className="error-message">{run.error_summary}</p> : null}<button type="button" className="link-button" onClick={() => void inspectRun(run.id)}>Review discovery evidence</button></article>)}</div>{selectedRun ? <div className="guidance-card"><strong>Discovery evidence — {new Date(selectedRun.run.checked_at).toLocaleString()}</strong><div className="standard-list">{selectedRun.items.map((item) => <div className="standard-option" key={item.id}><span className="badge">{item.result_state}</span><span><strong>{item.category_name ?? item.source_key}</strong><span className="standard-text">{item.observed_title ?? item.previous_title ?? item.source_key}</span><small>{item.authority} · {item.observed_edition ?? item.previous_edition ?? "edition unavailable"}</small></span></div>)}</div></div> : null}</section>
