...
async function readError(response: Response, fallback: string): Promise<string> { try { const body = await response.json() as { detail?: unknown }; return typeof body.detail === "string" && body.detail.trim() ? body.detail : fallback; } catch { return fallback; } }
function storedGradingPeriod(): { start: string; end: string } { try { const value = window.localStorage.getItem("tpp:current-grading-period"); if (!value) return { start: "", end: "" }; const parsed = JSON.parse(value) as { start?: unknown; end?: unknown }; return { start: typeof parsed.start === "string" ? parsed.start : "", end: typeof parsed.end === "string" ? parsed.end : "" }; } catch { return { start: "", end: "" }; } }
...
export function AdministrationOverview({ accessToken, roles, disabled = false }: Props) {
  ...
  async function loadUsage() {
    if (!period.start || !period.end) { setUsage(null); return; }
    setLoading(true); setError("");
    try {
      const query = new URLSearchParams({ period_start: period.start, period_end: period.end });
      const response = await fetch(`/api/v1/administration/usage?${query.toString()}`, { headers: { Authorization: `Bearer ${accessToken}` } });
    ...
    } catch (caught) { setUsage(null); setError(caught instanceof Error ? caught.message : "Administration reporting could not be loaded."); }
  ...
  async function loadCosts() {
    ...
    } catch (caught) { setError(caught instanceof Error ? caught.message : "Cost reporting could not be loaded."); }
  }
  ...
  function saveGradingPeriod() {
    if (!gradingStart || !gradingEnd || gradingEnd < gradingStart) { setError("Enter the district grading-period start and end dates before using this reporting period."); return; }
    window.localStorage.setItem("tpp:current-grading-period", JSON.stringify({ start: gradingStart, end: gradingEnd }));
    setError(""); void loadUsage();
  }
  ...
  return (
    <section className="panel administration-overview">
      ...
      {isPlatformAdmin && <><StandardsAdministrationPanel accessToken={accessToken} disabled={disabled || loading} /><section><div className="section-heading compact"><div><p className="eyebrow">Platform Owner</p><h2>AI cost reporting</h2><p className="supporting">Operational AI usage and estimated cost. A failed request means the planning request did not return a usable governed result; this summary does not infer the technical cause.</p></div></div>{costs.length === 0 ? <div className="empty-state"><p>No AI usage has been recorded.</p></div> : <div className="grid">{costs.map((cost) => <article className="card" key={`${cost.school_id}-${cost.usage_month}`}><span className="badge">{cost.usage_month.slice(0, 7)}</span><h3>${cost.estimated_cost_usd}</h3><p>{cost.request_count} requests · {cost.successful_requests} successful · {cost.failed_requests} failed</p><small>{cost.input_tokens} input · {cost.output_tokens} output · {cost.cached_tokens} cached tokens</small></article>)}</div>}</section></>}
    </section>
  );
}
