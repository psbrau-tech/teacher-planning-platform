...
function buildBaselineSummaries(results: BaselineResult[]): BaselineSchoolSummary[] {
  ...
  return [...grouped.entries()].map(([schoolId, responses]) => {
    ...
    const rarelyReviewReflection = responses.filter((item) => ["never", "rarely"].includes(item.reflection_review_frequency_before)).length;
    const rarelyUseInPlc = responses.filter((item) => ["never", "rarely"].includes(item.plc_use_frequency_before)).length;
    return {
      schoolId,
        ...
        <div role="tabpanel" aria-label="Administration reporting">
          <div className="section-heading compact">
            <div>
              ...
              <p className="supporting">Professional educator planning operations only.</p>
            </div>
          </div>
          <div className="report-period-control">
            ...
            {periodKind === "grading_period" && (
              <>
                <label>Grading period start<input type="date" value={gradingStart} onChange={(event) => setGradingStart(event.target.value)} /></label>
                <label>Grading period end<input type="date" value={gradingEnd} onChange={(event) => setGradingEnd(event.target.value)} /></label>
                <button type="button" className="secondary" onClick={saveGradingPeriod}>Save grading-period dates</button>
              </>
            )}
            <div className="guidance-card compact-guidance period-summary">
              <strong>{period.label}</strong>
              <p>{period.start && period.end ? `${period.start} through ${period.end}` : "Enter the district's actual grading-period dates."}</p>
            </div>
          </div>
          {periodKind === "grading_period" && (!gradingStart || !gradingEnd) && (
            <p className="guidance-text">TPP does not currently store district grading-period boundaries. Enter the district's actual dates once; this browser will remember them until a governed calendar configuration is added.</p>
          )}
          ...
          {usage ? (
            <>
              <section className="summary" aria-label="School planning usage">
                <div><strong>{usage.teachers_configured}</strong><span>teachers configured</span></div>
                <div><strong>{usage.teachers_with_assignments}</strong><span>teachers with courses</span></div>
                <div><strong>{usage.assignments_configured}</strong><span>courses configured</span></div>
                <div><strong>{usage.weekly_plans_created}</strong><span>weekly plans in period</span></div>
              </section>
              <div className="grid admin-activity-grid">
                <article className="card">
                  <h3>Weekly validation</h3>
                  <p>{usage.weekly_plans_approved} submitted plans</p>
                  <p>{usage.instruction_records_validated} instruction records validated</p>
                  <p>{usage.lessons_carried_forward} lessons carried forward</p>
                </article>
                <article className="card">
                  <h3>Document generation</h3>
                  <p>{usage.documents_requested} requested</p>
                  <p>{usage.documents_generated} generated</p>
                  <p>{usage.document_generation_failures} failures</p>
                </article>
              </div>
            </>
          ) : !loading && period.start && period.end ? (
            <div className="empty-state"><p>Reporting is unavailable for this period.</p></div>
          ) : null}
        ...
        <div className="owner-tab" role="tabpanel" aria-label="Platform Owner reporting">
          <div className="section-heading compact">
            <div>
              ...
              <h2>Owner</h2>
              <p className="supporting">Product adoption, Pilot learning, baseline evidence, governed standards operations, and AI cost reporting in one place.</p>
            </div>
            <button type="button" className="secondary" disabled={ownerLoading} onClick={() => void loadOwnerReporting()}>
              {ownerLoading ? "Refreshing…" : "Refresh owner data"}
            </button>
          </div>
          ...
          <section className="owner-section">
            <div className="section-heading compact">
              <div>
                <p className="eyebrow">Product and Pilot intelligence</p>
                <h3>Usage and teacher feedback</h3>
                <p className="supporting">These are product-learning tools, not teacher-performance measures.</p>
              </div>
            </div>
            <div className="grid owner-tool-grid">
              <article className="card owner-tool-card">
                <p className="eyebrow">Product adoption</p>
                <h3>Product usage</h3>
                <p>See what teachers are actually using, where adoption is growing, and where the product may be creating friction.</p>
                <ProductOwnerDashboardExperience />
              </article>
              <PilotFeedbackResultsPanel accessToken={accessToken} disabled={disabled || ownerLoading} />
            </div>
          </section>
          ...
          <section className="owner-section">
            <div className="section-heading compact">
              <div>
                <p className="eyebrow">Pre-TPP baseline</p>
                <h3>What planning looked like before TPP</h3>
                <p className="supporting">Responses are intentionally reported without teacher identity. They provide a comparison point for later workload and value measures.</p>
              </div>
            </div>
            {ownerLoading && baselineResults.length === 0 ? (
              <p className="working-status" role="status">Loading teacher baseline…</p>
            ) : baselineSummaries.length === 0 ? (
              <div className="empty-state"><p>No teacher baseline responses have been submitted yet.</p></div>
            ) : (
                ...
                {baselineSummaries.map((summary) => (
                  <article className="card baseline-owner-school" key={summary.schoolId}>
                    <div className="card-row">
                      <div><p className="eyebrow">School baseline</p><h3>{summary.schoolName}</h3></div>
                      <span className="badge">{summary.responses.length} response{summary.responses.length === 1 ? "" : "s"}</span>
                    </div>
                    ...
                    {summary.responses.some((item) => item.biggest_burden_before) && (
                      <div className="baseline-owner-comments">
                        ...
                        {summary.responses.filter((item) => item.biggest_burden_before).map((item) => (
                          <p key={item.id}>{item.biggest_burden_before}</p>
                        ))}
                      </div>
                    )}
                  </article>
          ...
          <section className="owner-section">
            <div className="section-heading compact">
              <div>
                <p className="eyebrow">Platform Owner</p>
                <h3>AI cost reporting</h3>
                <p className="supporting">Operational AI usage and estimated cost. A failed request means the planning request did not return a usable governed result; this summary does not infer the technical cause.</p>
              </div>
            </div>
            {costs.length === 0 ? (
              <div className="empty-state"><p>No AI usage has been recorded.</p></div>
            ) : (
              <div className="grid">
                {costs.map((cost) => (
