...
export function CourseSetupPanel({
  ...
  async function removeClass(assignment: Assignment) {
    if (!window.confirm(`Remove ${assignment.course_name} from active planning?\n\nExisting planning, submitted packets, and reusable curricula will be preserved.`)) return;
    setWorking(true);
    onError("");
    try {
      ...
      onMessage(`${assignment.course_name} removed from active planning. Its planning history is preserved.`);
    } catch (caught) {
  ...
  async function retireCurriculum(curriculum: Curriculum) {
    if (!window.confirm(`Remove ${curriculum.name} · ${curriculum.version} from your active curriculum list?\n\nThis does not delete historical planning records.`)) return;
    setWorking(true);
    onError("");
    try {
      ...
      if (!response.ok) throw new Error(await readError(response, "Curriculum could not be removed."));
  ...
  async function savePacing(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setPacingMode(null);
      onMessage(`${rows.length} pacing lesson${rows.length === 1 ? "" : "s"} saved and attached to ${updated.course_name}. Step 2 complete.`);
    } catch (caught) {
